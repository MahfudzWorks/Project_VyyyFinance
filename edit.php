...
<head>
  ...
  <title>Edit Transaksi - Project_VyyyFinance</title>
      ...
      <form method="POST" action="edit.php?id=<?= $id ?>" class="space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" name="tanggal" value="<?= sanitize($data['tanggal']) ?>" required class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          </div>
        ...
        <div>
          ...
          <input type="text" name="keterangan" value="<?= sanitize($data['keterangan']) ?>" required class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1">Kategori *</label>
            <input type="text" name="kategori" value="<?= sanitize($data['kategori']) ?>" required class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1">Jumlah (Rp) *</label>
            <input type="number" step="0.01" name="jumlah" value="<?= sanitize($data['jumlah']) ?>" required class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          </div>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-600 mb-1">Catatan Tambahan</label>
          <textarea name="catatan" rows="3" class="w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= sanitize($data['catatan']) ?></textarea>
        </div>

        <div class="pt-4 flex space-x-3">
          <button type="submit" class="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-medium py-2.5 rounded-lg text-sm transition">Perbarui Transaksi</button>
